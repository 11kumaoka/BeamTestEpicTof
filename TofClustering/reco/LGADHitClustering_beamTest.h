...
using LGADHitClusteringAlgorithm_beamTest =
    algorithms::Algorithm<algorithms::Input<edm4eic::TrackerHitCollection>,
                          algorithms::Output<edm4eic::Measurement2DCollection>>;

class LGADHitClustering_beamTest : public LGADHitClusteringAlgorithm_beamTest,
                          public WithPodConfig<LGADHitClusteringConfig_beamTest> {

public:
  LGADHitClustering_beamTest(std::string_view name)
      : LGADHitClusteringAlgorithm_beamTest{name, {"TOFBarrelCalHit"}, {"TOFBarrelRecHit"}, ""} {};
...
private:
  ...
  const Acts::Surface* m_fixed_surface = nullptr;
  ...
  dd4hep::DDSegmentation::Vector3D getBeamTestPosition(Int_t cellID) const;                      
