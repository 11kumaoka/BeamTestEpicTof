...
LGADHitClustering_beamTest::getBeamTestPosition(Int_t cellID) const {
  double pitch = 0.5 * dd4hep::mm;

  double x = 0.0;
  if(cellID % 8 == 0) x = - 2.0; // mm
  else if(cellID % 8 == 1) x = - 1.5; // mm
  else if(cellID % 8 == 2) x = - 1.0; // mm
  else if(cellID % 8 == 3) x = - 0.5; // mm
  else if(cellID % 8 == 4) x = 0.5; // mm
  else if(cellID % 8 == 5) x = 1.0; // mm
  else if(cellID % 8 == 6) x = 1.5; // mm
  else if(cellID % 8 == 7) x = 2.0; // mm
  
  double y = 0.0;
  if(cellID<8) y = - 10.0; // mm
  else y = 10.0; // mm

  double z = 0.0;

  return dd4hep::DDSegmentation::Vector3D(x, y, z);
}
