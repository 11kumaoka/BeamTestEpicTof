...
#include "LGADHitClustering_beamTest.h"
...
#include "LGADHitClusteringConfig_beamTest.h"

namespace eicrecon {

void LGADHitClustering_beamTest::init() {
  ...
  // m_acts_context = algorithms::ActsSvc::instance().acts_geometry_provider();　// Kuma Modif

}

void LGADHitClustering_beamTest::_calcCluster(const Output& output,
  ...
  for (size_t id = 0; id < hits.size(); ++id) {
    ...
    // ave_x += Edep * pos.x();//Orig
    // ave_y += Edep * pos.y();//Orig

    ave_x += Edep * hit.getPosition().x;
    ave_y += Edep * hit.getPosition().y;
    // std::cout << "Edep: " << Edep << ", pos.x: " <<hit.getPosition().x << ", pos.y: " << hit.getPosition().y << std::endl;
    ...
    if (Edep > max_charge) {
      ...
      max_charge_x        = hit.getPosition().x;
      max_charge_y        = hit.getPosition().y;
      // max_charge_x        = pos.x();//Orig
      // max_charge_y        = pos.y();//Orig
  ...
  edm4hep::Vector2f locPos{static_cast<float>(ave_x), static_cast<float>(ave_y)};
  // std::cout << "Cluster locPos x: " << ave_x << ", locPos y: " << ave_y << std::endl;

  // == s == === Original code to get surface from Acts geometry ===
  ...
  // == e == === Original code to get surface from Acts geometry ===

  cluster.setSurface(0); //Kuma Modif: set surface to 0 since we don't have Acts geometry for the beam test setup
...
void LGADHitClustering_beamTest::process(const LGADHitClustering_beamTest::Input& input,
                                const LGADHitClustering_beamTest::Output& output) const {
  ...
  for (auto [cellID, hitIDs] : hitIDsByCells) {
    ...
    // std::cout << "Finding neighbors for cellID: " << cellID << std::endl; //KUma Modif
    const auto* segmentation = getLocalSegmentation(cellID);//KUma Modif
    std::set<dd4hep::rec::CellID> cellNeighbors;//KUma Modif
    segmentation->neighbours(cellID, cellNeighbors);//KUma Modif
      ...
      if (it != hitIDsByCells.end()) {
        for (const auto& hitID1 : hitIDs) {
          for (const auto& hitID2 : it->second) {
            ...
            // std::cout << "Comparing hit " << hitID1 << " and hit " << hitID2
            //           << ", time difference: " << std::fabs(hit1.getTime() - hit2.getTime()) \
            //           << ", m_deltaT: " << m_cfg.deltaT << std::endl;
            // if (std::fabs(hit1.getTime() - hit2.getTime()) < m_cfg.deltaT) {
            if (std::fabs(hit1.getTime() - hit2.getTime()) < 3000.) { // ModifKuma
...
LGADHitClustering_beamTest::getLocalSegmentation(const dd4hep::rec::CellID& cellID) const {
...
LGADHitClustering_beamTest::UnionFind::UnionFind(int n) : mParent(n, 0), mRank(n, 0) {
  for (int i = 0; i < n; ++i) {
    mParent[i] = i;
  }
}

int LGADHitClustering_beamTest::UnionFind::find(int id) {
  if (mParent[id] == id) {
    return id;
  }
...
void LGADHitClustering_beamTest::UnionFind::merge(int id1, int id2) {
  auto root1 = find(id1);
  auto root2 = find(id2);
