...
void LGADHitCalibration_beamTest::process(const LGADHitCalibration_beamTest::Input& input,
  ...
  for (const auto& TDCADC_hit : *TDCADC_hits) {
    ...
    if(rawID > 7) continue; // Kuma Modif 
    ...
    //     id,
    //     edm4hep::Vector3f{
    //       static_cast<float>(pos.x()*0.5 - 2.0),
    //       static_cast<float>(20.0 * (pos.y()/8) - 10.0),
    ...
    // std::cout << "hit x: y: z = " << hit.getPosition().x << ", " << hit.getPosition().y << ", " << hit.getPosition().z
    //           << ", time: " << hit.getTime() << ", charge: " << hit.getEdep() << std::endl;


    auto hit = calibrated_hits->create(
        id,
        edm4hep::Vector3f{
          static_cast<float>(rawID*0.5 - 2.0),
          static_cast<float>(20.0 * (rawID/8) - 10.0),
